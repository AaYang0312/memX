...
- **日期：** 2026-09-23
- **状态：** 个人版 Task 1 开发已获项目使用者明确授权（2026-09-23）；**不是**原生产规格的 `Approved for implementation`，不代替组织/法务审批或 G9 四方签署。
- **依据：** 使用者提供 `https://github.com/AaYang0312/memX.git`，明确选择「保留公开，仅源码」「保留原架构」「允许外部模型」；随后明确批准「按 ADR 0005 启动个人版 Task 1；授权修订实施计划并决定本地技术版本；先用合成数据开发，真实记忆外发模型须另行确认」。GitHub API 已核实该仓库为 PUBLIC，原有初始提交仅含 MIT `LICENSE`。这些事实只支持以下**个人阶段**决策，不推导生产许可或个人数据外发许可。
...
6. **依赖与服务端许可不因个人用途自动消失。** 引入 Go 依赖、下载/使用 PG/Kafka/Redis/ES/Qdrant 镜像时仍核验版本兼容与相应许可；ADR 0004 §9 是生产候选而非批准。个人版 Task 1 的本地镜像摘要与选择记录于 ADR 0006，只用于静态 Compose 配置与合成数据；Go 客户端依赖仍按实际引入时逐项核验。特别是 Redis、Elasticsearch 的服务端许可不能由客户端许可推定；本地选择 Apache Kafka 而非 Redpanda 减少兼容/许可分歧。
