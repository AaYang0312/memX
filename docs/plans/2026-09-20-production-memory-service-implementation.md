...
### 6.6 个人版开发启动例外（2026-09-23；不改变 §6.4 生产验收）

项目使用者明确批准：**「按 ADR 0005 启动个人版 Task 1；授权修订实施计划并决定本地技术版本；先用合成数据开发，真实记忆外发模型须另行确认。」** 本段是个人开发轨道的启动授权，不是 §6.4 的生产 Task 0 验收、G9 四方签署或设计规格状态变更。GitHub 仓库公开但只提交源码/审过的文档；个人阶段版本及镜像摘要记录于 ADR 0006。未完成身份/RLS 与删除闭环前不开放业务 API 或接真实记忆数据；G9 草案可做信息性实验，不能用于发布通过结论。原多租户架构和 §23.2 的生产门禁保留不变。
...
### 7.7 个人版 Task 1 的独立验收边界（§6.6 授权）

- 仅在本地合成环境与只读 CI 实现健康检查/配置/工程骨架；不得把当前纯内存域原型当作 Task 2 契约验收。
- 本地 Go 精确版本及 PG/Kafka/Redis/ES/Qdrant 镜像 digest 依 ADR 0006 固定；Compose 只静态验证，不默认拉取或启动，真实联调按许可证及环境风险另行执行。
- `make check`（支持 make 的环境）或等价的 PowerShell 本地检查执行格式、`go test ./...`、`go vet ./...`；CI 只读、无生产 secret。`golangci-lint` 在选定并安装可核验版本前不伪称运行通过。
- 已配置但尚无真实 canonical PG 探针时 readiness 必须保持失败；不会因仅可连接 TCP 端口就报告 ready。没有业务表、记忆写入、对公网监听或真实数据外发。
- §7.5 是原生产验收，个人版通过本小节不等于 §6.4/§23.2 的生产门禁关闭。
...
Task 0 的完成标准不是凭空猜完所有业务参数，而是：已确认项写入 ADR；未确认项具有明确 owner、最晚关闭 Task 和可执行的 fail-closed 默认；所有 Task 1 阻塞项已关闭。**本表约束生产轨道；个人版 Task 1 可按 §6.6 / ADR 0005/0006 独立推进，不得标记本表已全部关闭。**
